...
class ftps{
  ...
  * @param integer $port  服务器端口，默认为21
  ...
  */
  public function connect($host, $username='', $password='', $port='21', $pasv=false, $ssl=false, $timeout=30){
    ...
    register_shutdown_function(array(&$this, 'close'));  //注册一个关机时调用的函数
  }
  ...
  */
  public function mkdir($dirname){
    ...
    $dirname = $this->ck_dirname($dirname);  //检查目录名
    $nowdir = '/';
    foreach ($dirname as $key => $value) {
      if($value && !$this->chdir($nowdir.$value)){
        if ($nowdir) {
          $this->chdir($nowdir);
        }
        @ftp_mkdir($this->link, $value);
      }
      if($value) $nowdir .= $v.'/';
    }
  }
  ...
  */
  ...
  /**
  * 在 FTP 服务器上改变当前目录  类似cd操作
  * @param string $dirname 修改服务器上的当前目录
  */
