...
final class cache_factory{
  ...
  */
  ...
  /**
  * 获取对象实例
  * @param $cache_config 缓存配置信息
  * @return object
  */
  public static function get_instance($cache_config = ''){
    if(!(self::$cache_factory instanceof self)){
      self::$cache_factory = new self;
    }
    if (!empty($cache_config)) {
      self::$cache_factory->cache_config = $cache_config;
    }
  ...
  public function load($cache_name){
    ...
    if (isset($this->cache_config[$cache_name]['type'])) {
      switch ($this->cache_config[$cache_name]['type']) {
        case 'file':
          $object = mp_base::load_sys_class('cache_file');
          break;
        case 'memcache':
          ...
          $object = mp_base::load_sys_class('cache_memcache');
          break;
        default:
          $object = mp_base::load_sys_class('cache_file');
          break;
